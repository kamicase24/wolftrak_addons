...
class WolftrakReport607(models.Model):
    ...
    def total_calculated(self):
        ...
        self.total_inv = ''.zfill(
            13)[len(str_total_inv[:str_total_inv.index('.')]):]+str_total_inv
    ...
    def _set_period(self):
        month = self.to_607.month
        year = self.to_607.year
        self.period = year + month

    @api.depends('from_607')
    def _set_from(self):
        year = self.from_607.year
        month = self.from_607.month
        day = self.from_607.day
    ...
    name = fields.Char(string='Reporte 607', copy=False)
    from_607 = fields.Date(
        string='Desde', default=time.strftime('%Y-%m-01'), copy=False)
    from_str = fields.Char(compute=_set_from)
    to_607 = fields.Date(string='Hasta',
                         default=str(datetime.now() + relativedelta.relativedelta(months=+1, day=1, days=-1))[:10], copy=False)
    to_str = fields.Char(compute=_set_to)
    total_inv = fields.Char(string='Total Calculado', copy=False)
    total_tax = fields.Char(string='ITBIS Calculado', copy=False)
    invoices = fields.Many2many('account.invoice', string='Facturas', domain=[('type', '=', 'out_invoice'),
                                                                              ('state', '!=', 'draft')], copy=False)
    period = fields.Char(string='Periodo', copy=False)
    number_reg = fields.Char('Cantidad de registros', copy=False)
    binary_report = fields.Binary(string='Descargar', copy=False)
    binary_string = fields.Char(string='Descargar', copy=False)

    def to_wizard(self):

        view_ref = self.env['ir.model.data'].get_object_reference(
            'wolftrakglobal', 'wizard_report607_view')
    ...
    def create(self, vals):
        sequence_id = self.env.ref('wolftrakglobal.report_607_sequence')
        vals.update({
            'name': sequence_id.next_by_id()
        })

        return super(WolftrakReport607, self).create(vals)


class WizardReport607(models.Model):
    _name = 'wizard.report607'
    _description = 'Wizard Reporte 607'

    def _default_report(self):
        print(self.env.context)
        report_id = self.env.context.get('report_id', False)
        if not report_id:
            report_id = self.env.context.get('active_id', False)
        return self.env['wolftrakglobal.report607'].search([('id', '=', report_id)])

    def _default_report_result(self):

        rpt = self.env['wolftrakglobal.report607'].search(
            [('id', '=', self.env.context['report_id'])])
        line1 = "607  131104371"+str(rpt.period) + \
            str(rpt.number_reg)+str(rpt.total_inv)+"\n"
        for inv in rpt.invoices:
            line2 = "  "+str(inv.partner_id.doc_ident) + \
                str(inv.partner_id.doc_ident_type)+str(inv.ncf)
            date = str(inv.date)
            ...
            line2 += "".zfill(9)[len(str_amount_tax[:str_amount_tax.index('.')])
                              :]+str_amount_tax
            line2 += "".zfill(9)[len(str_amount_untaxed[:str_amount_untaxed.index('.')])
                              :]+str_amount_untaxed+"\n"
            line1 += line2
        return line1

    report_result = fields.Text(
        string="Reporte", default=_default_report_result)
    report_id = fields.Many2one(
        'wolftrakglobal.report607', default=_default_report)
    path = fields.Char(string='Ruta')

    def download_file(self):
        if not self.path:
            raise ValidationError(
                'Por favor proporcione una ruta para el archivo.')
        content_write = open(f"{self.path}new_file.txt", "w+")
        content_write.write(self.report_result)
        content_write.close()
        content_read = open(f"{self.path}new_file.txt", "rb")
        out = content_read.read()
        self.report_id.write({
            'binary_string': f'{self.report_id.name}/{self.report_id.period}.txt',
            'binary_report': base64.b64encode(out)
        })


class WolftrakReport606(models.Model):
    _name = 'wolftrakglobal.report606'
    _description = 'Reporte 606'

    @api.onchange('to_606', 'moves')
    def _set_period(self):
        month = self.to_606.month
        year = self.to_606.year
        self.period = year+month

    @api.depends('from_606', 'to_606')
    @api.onchange('from_606', 'to_606')
    def _set_dates(self):
        from_year = self.from_606.year
        from_month = self.from_606.month
        from_day = self.from_606.day
        self.form_str = from_year + from_month + from_day

        to_year = self.to_606.year
        to_month = self.to_606.month
        to_day = self.to_606.day
    ...
    def total_calculated(self):
        ...
        self.total_inv = ''.zfill(
            13)[len(str_total_inv[:str_total_inv.index('.')]):]+str_total_inv
        self.total_tax = ''.zfill(
            9)[len(str_total_tax[:str_total_tax.index('.')]):]+str_total_tax
        self.total_tax_hold = ''.zfill(
            9)[len(str_total_tax_hold[:str_total_tax_hold.index('.')]):]+str_total_tax_hold
    ...
    name = fields.Char(string='Reporte 606', copy=False)
    from_606 = fields.Date(
        string='Desde', default=time.strftime('%Y-%m-01'), copy=False)
    from_str = fields.Char(compute=_set_dates, copy=False)
    to_606 = fields.Date(string='Hasta',
                         default=str(datetime.now() + relativedelta.relativedelta(months=+1, day=1, days=-1))[:10])
    to_str = fields.Char(compute=_set_dates)
    period = fields.Char(string='Periodo', copy=False)
    number_reg = fields.Char('Cantidad de registros', copy=False)
    total_tax_hold = fields.Char('ITBIS Retenido', copy=False)
    total_tax = fields.Char('ITBIS Calculado', copy=False)
    total_inv = fields.Char('Total Calculado', copy=False)
    moves = fields.Many2many('account.move', string='Asientos', domain=[
                             ('journal_id', '=', 2)], copy=False)
    binary_report = fields.Binary(string='Descargar', copy=False)
    binary_string = fields.Char(string='Descargar', copy=False)
    ...
    def to_wizard(self):
        ...
        view_ref = self.env['ir.model.data'].get_object_reference(
            'wolftrakglobal', 'wizard_report606_view')
        view_id = view_ref[1] if view_ref else False
    ...
    @api.model
    def create(self, vals):
        sequence_id = self.env.ref('wolftrakglobal.report_606_sequence')
        vals.update({
            'name': sequence_id.next_by_id()
        })

        return super(WolftrakReport606, self).create(vals)


class WizardReport606(models.Model):
    _name = 'wizard.report606'
    _description = 'Wizard Reporte 606'
    ...
    def _default_report(self):
        ...
        if not report_id:
            report_id = self.env.context.get('active_id', False)
        return self.env['wolftrakglobal.report606'].search([('id', '=', report_id)])

    def _default_report_result(self):
        rpt = self.env['wolftrakglobal.report606'].search(
            [('id', '=', self.env.context['report_id'])])

        var1 = "606  131104371" + \
            str(rpt.period)+str(rpt.number_reg) + \
            str(rpt.total_inv)+str(rpt.total_tax_hold)+"\n"
        for move in rpt.moves:
            var1 += str(move.partner_id.doc_ident)+"  "+str(move.partner_id.doc_ident_type) + \
                str(move.type_buy)+str(move.ncf)+"                   "
            date = str(move.date)
            year = date[:date.index('-')]
            rest = date[date.index('-')+1:]
            date_result = year+rest[:rest.index('-')]+rest[rest.index('-')+1:]
            ...
            var1 += date_result+date_result + \
                ''.zfill(9)[len(str_tax[:str_tax.index('.')]):]+str_tax
            # itbis retenido
            var1 += ''.zfill(9)[len(str_tax_hold[:str_tax_hold.index('.')])
                             :]+str_tax_hold
            # monto factura
            var1 += ''.zfill(9)[len(str_amount[:str_amount.index('.')])
                             :]+str_amount
            # retencion renta
            var1 += ''.zfill(9)[len(str_rent_hold[:str_rent_hold.index('.')])
                             :]+str_rent_hold+'\n'

        return var1

    report_id = fields.Many2one(
        'wolftrakglobal.report606', default=_default_report)
    report_result = fields.Text(
        string="Reporte", default=_default_report_result)
    path = fields.Char(string='Ruta')
...
class WizardPartnerReport(models.TransientModel):
    _name = 'wizard.partner.report'
    _description = 'Wizard Reporte de Cliene'
    ...
    def update_lines(self):
        ...
        for name in partner_names:
            for inv in invoices:
                if inv.partner_id.name == name:
                    ...
                    partner_complete.append(
                        {inv.partner_id.name: [inv.amount_total, inv_lines]})
        ...
        view_ref = self.env['ir.model.data'].get_object_reference(
            'wolftrakglobal', 'wizard_partner_report_form')
        view_id = view_ref[1] if view_ref else False
...
class WolftrakPartnersDashboard(models.Model):
    ...
    partner_report = fields.Many2many(
        'partner.report', string='Reporte', store=True)
...
class WolftrakDebtClientsDashboard(models.Model):
    ...
    debt_clients_report = fields.One2many(
        'debt.clients.report', 'dashboard_id', string='Cuentas por Cobrar')
...
class WolftrakDebtClientsReport(models.Model):
    ...
    def _compute_due_days(self):
        for line in self:
            date_due = datetime(int(line.date_due[:4]), int(
                line.date_due[5:7]), int(line.date_due[-2:]))
    ...
    amount_pay = fields.Float(
        string='Abonado', compute=_compute_amonut_pay, store=True)
    ...
    due_days = fields.Integer(string='Dias Vencidos',
                              compute=_compute_due_days, store=True)
