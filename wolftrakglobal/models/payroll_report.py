...
class WolftrakPayrollReport(models.Model):
    ...
    _description = 'Reporte de Nomina Quincenal'
    ...
    employee_ids = fields.Many2many('hr.employee', string='Empleados', default=_default_employees)
    payslip_ids = fields.Many2many('hr.payslip', string='Nómina')
    payslip_lines = fields.Many2many('hr.payslip.line', string='Lineas de Nómina')

    def load_payslip(self):
        payslip_ids = self.env['hr.payslip'].search([('date_from', '=>', self.date_from), ('date_to', '=<', self.date_to)])
        if len(payslip_ids) > 0: 
            self.payslip_ids = payslip_ids
        else: 
            raise UserError(_("Las fechas seleccionadas no coinciden con ninguna Nómina"))
    ...
    def _set_report(self):
        ...
        for emp in self.employee_ids:
            ps_id = ''
            for ps in self.payslip_ids:
                if ps.employee_ids.id == emp.id:
                    ps_id = ps.id
...
class WolftrakPayrollReportWizard(models.TransientModel):
    ...
    _description = 'Wizard Reporte de Nómina'
...
class WolftrakPayrollList(models.Model):
    ...
    _description = 'Detalle de reporte de Nómina'
